...
radio_button_tag 在view_helper.rb的重构
...
<pre>
git status 看下哪些文件中有要替换的文件名
sed -i 's/issue/inspect/g' `git status | grep inspect | grep '\.' | awk '{print  $2}'` 其中issue是原文件名,inspect是用于替换的文件名
</pre>

替换文本内的文字

<pre>
ls 查看哪些文件下有可能有要修改的文字
sed -i 's/Issue/Inspect/g' `grep -r -l Issue spec` 其中Issue是原文字,Inspect是替换文字, spec是含有Issue的文件
</pre>
