...
#安装rails及其他库
sudo gem install rails rspec hoe haml paperclip mislav-will_paginate mysql mongrel mongrel_cluster aslakhellesoy-cucumber webrat javan-whenever ferret acts_as_ferret rmmseg --no-ri --no-rdoc
#开发机额外安装(xosd-bin及以后的内容都是autotest所需)
sudo apt-get install irb ctags xosd-bin ruby-gnome2 libnotify-dev
sudo gem install rspec-rails ZenTest diff-lcs --no-ri --no-rdoc
...
安装浏览器开发插件
"Web Development":https://addons.mozilla.org/en-US/firefox/downloads/latest/60
...
"安装全文检索Ferret":http://wiki.github.com/cogentsoft/zbs/ferret
"安装短信机客户端环境":http://wiki.github.com/cogentsoft/zbs/192362
...
<code>
  ...
  #或者使用以下语句启动(以下方式启动时，打开的浏览器地址应为http://localhost:3000)
  mongrel_rails start
</code>
</pre>
打开浏览器，输入地址http://localhost:4000，是否看到首页了？
